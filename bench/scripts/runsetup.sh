...
process_argument() {
  ...
  rm -rf "benchspec/CPU/$arg/run"

  # Execute the runcpu command
  if [[ "$arg" == "620.omnetpp_s" || "$arg" == "520.omnetpp_r" ||  "$arg" == "508.namd_r" || "$arg" == "511.povray_r" ]]; then
    runcpu --config="${flags_underscored}cpp14.cfg" --tune=base --action=runsetup --iterations 1 "$arg"
  elif [[ "$arg" == "510.parest_r" ]]; then
    runcpu --config="${flags_underscored}cpp03.cfg" --tune=base --action=runsetup --iterations 1 "$arg"
  else
    runcpu --config="${flags_underscored}.cfg" --tune=base --action=runsetup --iterations 1 "$arg"
  fi
