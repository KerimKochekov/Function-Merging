...
#include "llvm/Transforms/IPO/SALSSA_FM.h"
...
FunctionMergeResult FunctionMerger::merge(Function* F1, Function* F2, std::string Name, const FunctionMergingOptions& Options) {
  ...
  /*
      ...
      default: {
        HirschbergSA<SmallVectorImpl<Value*>> SA(ScoringSystem(-1,2),FunctionMerger::match);
        AlignedSeq = SA.getAlignment(F1Vec,F2Vec);
        break;
      }
    }
...
bool SALSSA_FM::runOnModule(Module& M) {
#ifdef TIME_STEPS_DEBUG
  ...
  //errs() << "Running FMSA\n";
    ...
    pid = getpid();
#endif
    TPrefix = std::string("/tmp/") + std::to_string(pid) + std::string("-") + std::to_string((size_t)(&SALSSA_FM::ID)) + std::string(".");
    //TPrefix = std::string("../");
...
void SALSSA_FM::getAnalysisUsage(AnalysisUsage& AU) const {
...
char SALSSA_FM::ID = 0;
static RegisterPass<SALSSA_FM> X("salssa-fm", "SALSSA Function Merging", false, false);
