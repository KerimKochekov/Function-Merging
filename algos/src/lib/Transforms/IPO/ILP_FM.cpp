...
#include "llvm/Transforms/IPO/ILP_FM.h"
...
static bool matchIntrinsicCalls(Intrinsic::ID ID, const CallInst* CI1, const CallInst* CI2) {
    Intrinsic::ID ID1;
    Intrinsic::ID ID2;
...
static unsigned RandomLinearizationOfBlocks(BasicBlock* BB,
...
static unsigned RandomLinearizationOfBlocks(Function* F, std::list<BasicBlock*>& OrederedBBs) {
    std::set<BasicBlock*> Visited;
    return RandomLinearizationOfBlocks(&F->getEntryBlock(), OrederedBBs, Visited);
}

static unsigned CanonicalLinearizationOfBlocks(BasicBlock* BB,
    std::list<BasicBlock*>& OrederedBBs,
    std::set<BasicBlock*>& Visited) {
    if (Visited.find(BB) != Visited.end())
        return 0;
...
static unsigned CanonicalLinearizationOfBlocks(Function* F,
    std::list<BasicBlock*>& OrederedBBs) {
    std::set<BasicBlock*> Visited;
...
static bool runFMTest(Module& M, FunctionMerger& FM, TargetTransformInfo& TTI, FunctionMergingOptions Options = {}) {
    ...
    //Merges.push_back({"_ZN9flexarrayI6pointtE8doublingEb__merged", {"_ZN9flexarrayI6pointtE8doublingEb", "_ZN9flexarrayI11levelplacetE8doublingEb"}});
    //Merges.push_back({"_ZN9flexarrayIP6regobjE8doublingEb__merged", {"_ZN9flexarrayIP6regobjE8doublingEb", "_ZN9flexarrayI11regboundobjE8doublingEb"}});
    //Merges.push_back({"_ZN6wayobj9getxcoordEi__merged", {"_ZN6wayobj9getxcoordEi", "_ZN6wayobj9getycoordEi"}});
...
bool ILP_FM::runOnModule(Module& M) {
#ifdef TIME_STEPS_DEBUG
  ...
  //errs() << "Running FMSA\n";
    ...
    pid = getpid();
#endif
    TPrefix = std::string("/tmp/") + std::to_string(pid) + std::string("-") + std::to_string((size_t)(&ILP_FM::ID)) + std::string(".");
    //TPrefix = std::string("../");
    ...
    */
    ...
    if (RunPrediction) {
        errs() << "Timer:Prediction: " << TimePrediction.getTotalTime().getWallTime() << "\n";
...
void ILP_FM::getAnalysisUsage(AnalysisUsage& AU) const {
...
char ILP_FM::ID = 0;
static RegisterPass<ILP_FM> X("ilp-fm", "ILP-based Function Merging", false, false);
