...
static void CollectGlobalsInConst(const Constant* C, std::set<const GlobalObject*>& Globals, std::set<const Value*> NeededValues, std::set<const Value*>& Visited) {
...
static void CloneUsedGlobalsAcrossModule(const Function* F, const Module* M, Module* NewM, ValueToValueMapTy& VMap) {
...
static void CloneFunctionAcrossModule(const Function* F, Module* M, ValueToValueMapTy& VMap, bool FunctionOnly = false) {
...
static void ExtractFunctionIntoFile(Module& M, std::string FName, std::string FilePath) {
...
static std::unique_ptr<Module> ExtractMultipleFunctionsIntoNewModule(std::vector<Function*>& Fs, Module& M) {
    std::unique_ptr<Module> NewM =
        std::make_unique<Module>(M.getModuleIdentifier(), M.getContext());
...
static Optional<size_t> MeasureSize(std::vector<Function*>& Fs, Module& M, bool Timeout = true) {
...
static Optional<size_t> MeasureSize(Module& M, std::string Name, bool Timeout = true) {
...
static bool canReplaceCallsWith(Function* F, FunctionMergeResult& MFR, const FunctionMergingOptions& Options) {
...
static bool MeasureMergedSize(unsigned& SizeF1F2, unsigned& SizeF12, Module& M, FunctionMergeResult& Result, StringSet<>& AlwaysPreserved, const FunctionMergingOptions& Options = {}, bool Timeout = true) {
...
static Optional<size_t> MeasureSize(Module& M, FunctionMergeResult& Result, StringSet<>& AlwaysPreserved, const FunctionMergingOptions& Options = {}, bool Timeout = true) {
...
static void ReplaceByMergedBody(bool IsFunc1, Function* F, Function* MF, Module* M, std::map<unsigned, unsigned>& ArgMap, bool HasIdArg, ValueToValueMapTy& VMap) {
...
static void ExportForAlive(Function* F, Module& M, FunctionMergeResult& Result, std::string Suffix, const FunctionMergingOptions& Options = {}) {
    std::unique_ptr<Module> NewM =
        std::make_unique<Module>(M.getModuleIdentifier(), M.getContext());
    //std::unique_ptr<Module> NewM = std::make_unique<Module>(M.getModuleIdentifier(), M.getContext());
...
static void ExportForAlive(Module& M, FunctionMergeResult& Result, unsigned FileId, const FunctionMergingOptions& Options = {}) {
    Function* F1 = Result.getFunctions().first;
    Function* F2 = Result.getFunctions().second;
