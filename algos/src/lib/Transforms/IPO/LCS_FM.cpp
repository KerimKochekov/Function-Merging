...
#include "llvm/Transforms/IPO/LCS_FM.h"
...
using namespace LCS_globs;

static cl::opt<unsigned> ExplorationThreshold(
    "lcs-fm-explore", cl::init(1), cl::Hidden,
    cl::desc("Exploration threshold of evaluated functions"));


static cl::opt<unsigned> SAMethod(
    "lcs-fm-sa-method", cl::init(1), cl::Hidden,
    cl::desc("(0) hirschberg; (1) needleman-wunsch;"));

static cl::opt<unsigned> RankingThreshold(
    "lcs-fm-ranking-threshold", cl::init(0), cl::Hidden,
    cl::desc("Threshold of how many candidates should be ranked"));


static cl::opt<int> MergingOverheadThreshold(
    "lcs-fm-threshold", cl::init(0), cl::Hidden,
    cl::desc("Threshold of allowed overhead for merging function"));

static cl::opt<bool>
MaxParamScore("lcs-fm-max-param", cl::init(true), cl::Hidden,
    cl::desc("Maximizing the score for merging parameters"));

static cl::opt<bool> Debug("lcs-fm-debug", cl::init(true), cl::Hidden,
    cl::desc("Outputs debug information"));

static cl::opt<bool> Verbose("lcs-fm-verbose", cl::init(false),
    cl::Hidden, cl::desc("Outputs debug information"));

static cl::opt<bool>
IdenticalType("lcs-fm-identic-type", cl::init(true), cl::Hidden,
    cl::desc("Match only values with identical types"));

static cl::opt<bool> ApplySimilarityHeuristic(
    "lcs-fm-similarity-pruning", cl::init(false), cl::Hidden,
    cl::desc("Prune the candidates based on their fingerprint similarity"));

static cl::opt<bool>
EnableUnifiedReturnType("lcs-fm-unify-return", cl::init(false), cl::Hidden,
    cl::desc("Enable unified return types"));

static cl::opt<bool>
EnableOperandReordering("lcs-fm-operand-reorder", cl::init(true), cl::Hidden,
    cl::desc("Enable operand reordering"));

static cl::opt<bool>
HasWholeProgram("lcs-fm-whole-program", cl::init(true), cl::Hidden,
    cl::desc("Function merging applied on whole program"));

static cl::opt<bool>
EnableSALSSA("lcs-fm-salssa", cl::init(true), cl::Hidden,
    cl::desc("Enable full support of the SSA form with SalSSA"));

static cl::opt<bool>
EnableSALSSACoalescing("lcs-fm-coalescing", cl::init(true), cl::Hidden,
    cl::desc("Enable phi-node coalescing during SSA reconstruction"));


// static cl::opt<bool> ConservativeMode(
//     "lcs-fm-conservative", cl::init(false), cl::Hidden,
//     cl::desc("Enable conservative mode to avoid runtime overhead"));

// static cl::opt<bool> MergeListNames(
//     "lcs-fm-list-merges", cl::init(false), cl::Hidden,
//     cl::desc("Run function merging using the list of function names"));

// static cl::opt<bool> GenTrainingData(
//     "lcs-fm-gen-training", cl::init(false), cl::Hidden,
//     cl::desc("Generate the training data"));

static cl::opt<bool> GenTestingData(
    "lcs-fm-gen-testing", cl::init(false), cl::Hidden,
    cl::desc("Generate the testing data"));

// static cl::opt<bool> RunPrediction(
//     "lcs-fm-predict", cl::init(false), cl::Hidden,
//     cl::desc("Generate the testing data"));


static cl::opt<std::string> OptBenchName(
    "lcs-fm-bench-name", cl::init(""), cl::Hidden,
    cl::desc("Generate the testing data"));

static cl::opt<unsigned> MaxNumSelection(
    "lcs-fm-max-selects", cl::init(500), cl::Hidden,
    cl::desc("Maximum number of allowed operand selection"));

static cl::opt<unsigned> EnableLCS(
    "lcs-run", cl::init(true), cl::Hidden,
    cl::desc("Maximum number of allowed operand selection"));
    
static cl::opt<unsigned> RunStats(
    "lcs-run-stats", cl::init(false), cl::Hidden,
    cl::desc("Maximum number of allowed operand selection"));


//////////////////////////// Tests

static cl::opt<std::string> TestFM_DataPATH("lcs-fm-gen-prefix",
    cl::init(""), cl::Hidden, cl::desc(""));


// static cl::opt<bool> TestFM_Oracle("lcs-fm-built-oracle",
//     cl::init(false), cl::Hidden, cl::desc(""));

// static cl::opt<bool> TestFM_CompilationCostModel("lcs-fm-built-size-cost",
//     cl::init(false), cl::Hidden, cl::desc(""));

static cl::opt<std::string> TestFM_ClangPATH("lcs-fm-built-size-cost-cc",
    cl::init(""), cl::Hidden, cl::desc(""));
...
static bool matchCallInsts(const CallInst* CI1, const CallInst* CI2) {
...
static void MergeArguments(LLVMContext& Context, Function* F1, Function* F2, AlignedSequence<Value*>& AlignedSeq, std::map<unsigned, unsigned>& ParamMap1, std::map<unsigned, unsigned>& ParamMap2, std::vector<Type*>& Args, const FunctionMergingOptions& Options) {
...
template<typename BlockListType>
bool FunctionMerger::FMSACodeGen<BlockListType>::generate(AlignedSequence<Value*>& AlignedSeq,
...
FunctionMergeResult FunctionMerger::merge(Function* F1, Function* F2, std::string Name, const FunctionMergingOptions& Options) {
   ...
   if (useDP) {
        TreeStringAlignment <std::vector <Value*>> TSA(ScoringSystem(-1, 2), FunctionMerger::match);
        memo = TSA.getOptFlattening(F1, F2, F1Vec, F2Vec);
   } 
   if (F1Vec.empty() && F2Vec.empty()) {
        linearize(F1, F1Vec);
        linearize(F2, F2Vec);
   }
    // for (Value *I : F1Vec) errs() << (*I) << '\n';
    // for (Value *I : F2Vec) errs() << (*I) << '\n';
    ...
    AlignedSequence<Value*> AlignedSeq = SA.getAlignment(F1Vec, F2Vec);
...
static bool SimilarityHeuristicFilter(const FingerprintSimilarity& Item) {
...
struct MergeOrder {
    ...
    MergeOrder() {}
    MergeOrder(std::string ON, std::vector<std::string> FNs) : OutputName(ON), FNames(FNs) {}
};

// static bool runFMTest(Module& M, FunctionMerger& FM, TargetTransformInfo& TTI, FunctionMergingOptions Options = {}) {
...
//         if (Merged) Merged->setName(Order.OutputName);
//     }
...
static std::string GetTypeName(const Type* Ty) {
    ...
    if (const VectorType* VecTy = dyn_cast<VectorType>(Ty)) {
        return (std::string("vec_") + std::to_string(VecTy->getNumElements()) + std::string(" ") + GetTypeName(VecTy->getElementType()));
    }
    if (const ArrayType* ArrTy = dyn_cast<ArrayType>(Ty)) {
        return (std::string("arr_") + std::to_string(ArrTy->getNumElements()) + std::string(" ") + GetTypeName(ArrTy->getElementType()));
    }
...
bool LCS_FM::runOnModule(Module& M) {
    errs()<< "Running LCS Function Merger\n";
    ...
    // if (MergeListNames) return runFMTest(M, FM, TTI, Options);
    ...
    for (auto& F : M) {
        ...
        for (int i = 0; i < node_count; i++){
            ...
            branchFactor = std::max(branchFactor, (int)Fstr->nodes[i].getBranches().size());
        }
        ...
        totalDepth += maxDepth; totalBranchFactor += branchFactor;
    ...
    if (RunStats){
        ...
        fprintf(outputfile, "%4d %7d %10d %10d %10d %10d",
            (int)FunctionsToProcess.size(),
            totalInstructions,
            totalDepth,
            totalBranchFactor,
            overallMaxDepth,
            overallMaxBranchFactor);
        for (int i = 0; i < 11; ++i)
    ...
    std::sort(FunctionsToProcess.begin(), FunctionsToProcess.end(), CompareFunctionScores);
    ...
    while (!WorkList.empty()) {
        ...
        while (!Rank.empty()) {
            ...
            useDP = 0;
            FunctionMergeResult Result = FM.merge(F1, F2, Name, Options);
            FunctionMergeResult DpResult = Result; 
    
            if (EnableLCS){
                useDP = 1;
                DpResult = FM.merge(F1, F2, Name, Options);
            }
            ...
            if (Result.getMergedFunction() != nullptr && verifyFunction(*Result.getMergedFunction())) {
                if (Debug) {
                    errs() << "Invalid Function: " << GetValueName(F1) << ", " << GetValueName(F2) << "\n";
                }
                Result.getMergedFunction()->eraseFromParent();
                if (DpResult.getMergedFunction() != nullptr && verifyFunction(*DpResult.getMergedFunction()))
                    DpResult.getMergedFunction()->eraseFromParent();
                  
                validFunction = false;
            }
            
            if (Result.getMergedFunction() and validFunction) {
                if (!EnableSALSSA) {
                    DominatorTree MergedDT(*Result.getMergedFunction());
                    promoteMemoryToRegister(*Result.getMergedFunction(), MergedDT);
                    DominatorTree MergedDpDT(*DpResult.getMergedFunction());
                    promoteMemoryToRegister(*DpResult.getMergedFunction(), MergedDpDT);
                }
                ...
                unsigned SizeF12 = EstimateThunkOverhead(Result, AlwaysPreserved) +
                    EstimateFunctionSize(Result.getMergedFunction(), &TTI);
                unsigned DpSizeF12 = EstimateThunkOverhead(DpResult, AlwaysPreserved) +
                    EstimateFunctionSize(DpResult.getMergedFunction(), &TTI);

                errs() << SizeF12 << " " << DpSizeF12 << "\n";
                ...
                // FILE *outputfile;
                // outputfile = fopen((OutputPrefix+"/part3.txt").c_str(), "a");
                // if (Profitable){
                ...
                if (Profitable) {
                    ...
                    if (!EnableSALSSA){
                        ...
                        demoteRegToMem(*DpResult.getMergedFunction());
                    }
                    CachedFingerprints[Result.getMergedFunction()] = new Fingerprint(Result.getMergedFunction());
                    break; 
                }
                else {
                    if (Result.getMergedFunction() != nullptr)
                        Result.getMergedFunction()->eraseFromParent();
                    if (DpResult.getMergedFunction() != nullptr)
                        DpResult.getMergedFunction()->eraseFromParent();
                } 
...
void LCS_FM::getAnalysisUsage(AnalysisUsage& AU) const {
...
char LCS_FM::ID = 0;
static RegisterPass<LCS_FM> X("lcs-fm", "LCS-based Function Merging", false, false);

/*
INITIALIZE_PASS(FunctionMerging, "func-merging", "New Function Merging", false, false)
...
template<typename BlockListType>
bool FunctionMerger::SALSSACodeGen<BlockListType>::generate(AlignedSequence<Value*>& AlignedSeq,
    ValueToValueMapTy& VMap,
    const FunctionMergingOptions& Options) {
    ...
    CodeGen(Blocks1, Blocks2, EntryBB1, EntryBB2, MergedFunc, IsFunc1, PreBB, AlignedSeq, VMap, BlocksF1, BlocksF2, MaterialNodes);
            ...
            for (Instruction& I : *BB) {
                if (PHINode* PHI = dyn_cast<PHINode>(&I)) {
                    ...
                    for (auto It = pred_begin(NewPHI->getParent()),
                        ...
                        else V = GetAnyValue(NewPHI->getType());
    ...
    if (MergedFunc!=nullptr) {
      ...
      for (auto &Pair : MaterialNodes) if (!isa<BasicBlock>(Pair.first) && FuseBBsBackward(Pair.second)) DeleteBlocks.insert(Pair.second);
