...
private:
    ...
    /*  This class stores a structured (sub)string following T -> S | S [T ... T] T.
        ...
        - If isLeaf = false, this represents substring S | [br] | ch, 
          ...
          'branches stores' the sequence of substrings (sibling tree nodes) in [br], 
          and 'sibling' stores the substring (sibling tree node) representing ch. 
    */
public:
    ...
    void deleteLastBranch() { 
        ...
        if (branches.empty()) isLeaf = true;
    }
    void clearBranches() { branches.clear(); isLeaf = true; }
    ...
    void printNode() {
        ...
        if (!isLeaf) {
            ...
            for (int br : branches) {
                errs() << 'P' << br;
                if (br != branches.back()) errs() << ' ';
            }
            errs() << "] ";
            if (sibling != -1) errs() << 'P' << sibling;
            else errs() << "none";
        }
        errs() << '\n';
        errs() << "\t";
...
public:
    ...
    void confirmDepth(int limitDepth, int node) {
        assert(nodes[node].getDepth() < limitDepth && "Tree depth exceeds limit!");
        for (auto child_branch : nodes[node].getBranches())
    ...
    void confirmBranchingFactor(int limitBranchFactor, int node) {
        assert(nodes[node].getBranches().size() <= limitBranchFactor && "Branching factor exceeds limit!");
        for (auto child_branch : nodes[node].getBranches())
    ...
    void getTrimmedCFG(int u, std::vector <bool> &visited, std::vector <bool> &marked) {
        visited[u] = true;
        marked[u] = true;
    ...
    int buildTree(std::set <int> &curBlocks, int fa) {
        ...
        for (int x : curBlocks) {
            ...
            // classify blocks into different branches (and those succeeding all branches).
            // if a block is accessed by more than two branches (happens if switch or goto exists), 
            // classify it to the one with smallest color labeling (won't affect correctness).
            std::set <int> ch;
        ...
        if (totalBlocksAdded != curBlocks.size()){
            ...
            errs() << "node " << curNode << " has " << curBlocks.size() << " blocks, "
                   << "but its subtree contains " << totalBlocksAdded << " blocks in total!\n";
        }
        return curNode;
    }
...
class TreeStringAlignment : public SequenceAligner <ContainerType, Ty, Blank, MatchFnTy> {
private:
    ...
    std::vector<std::pair<std::pair<int, Signature>, int> > getNextMoves(int node, TreeString *Fstr, Signature S) {
        ...
        // Move to sibling
            ...
            moves.push_back({{curNode.getSibling(), S.allZero(curNode.getDepth())}, 0});
        // Move to father
        else {
            int tempNode = node;
            while (Fstr->nodes[tempNode].getElderSibling() != -1 and Fstr->nodes[tempNode].getFather() == -1)
                tempNode = Fstr->nodes[tempNode].getElderSibling();
            if (Fstr->nodes[tempNode].getFather() != -1)
                moves.push_back({{Fstr->nodes[tempNode].getFather(), S.allZero(curNode.getDepth())}, 1});
        }
        return moves;
    }
    // Compute match score between two blocks
    int matchScore(int node1, int node2, int pos1, int pos2) {
        return BaseType::match(F1str->nodes[node1].getFVecByPos(pos1), F2str->nodes[node2].getFVecByPos(pos2)) 
                ? BaseType::getScoring().getMatchProfit()
                : BaseType::getScoring().getMismatchPenalty();
    }

    int gapScore() {
        return BaseType::getScoring().getGapPenalty();
    }
    ...
    int rec(int node1, int node2, int pos1, int pos2, Signature S1, Signature S2, 
        std::vector<std::vector<std::vector<std::vector<std::vector<std::vector<int>>>>>> &dp,
        std::vector<std::vector<std::vector<std::vector<std::vector<std::vector<int>>>>>> &way) {
        ...
        if (pos1 < node1Size){
            int temp = rec(node1, node2, pos1+1, pos2, S1, S2, dp, way) + gapScore();
            if (temp > ret){
                ret = temp;
                way_ret = 1; // gap in F2
            }
        ...
        if (pos2 < node2Size){
            int temp = rec(node1, node2, pos1, pos2+1, S1, S2, dp, way) + gapScore();
            if (temp > ret){
                ret = temp;
                way_ret = 2; // gap in F1
            }
        ...
        if (pos1 < node1Size and pos2 < node2Size){
            int temp = rec(node1, node2, pos1+1, pos2+1, S1, S2, dp, way) + matchScore(node1, node2, pos1, pos2);
            if (temp > ret){
                ret = temp;
    ...
    void get_orderings_from_dp(int node1, int node2, int pos1, int pos2, Signature S1, Signature S2, 
        std::vector<std::vector<std::vector<std::vector<std::vector<std::vector<int>>>>>> &dp,
        std::vector<std::vector<std::vector<std::vector<std::vector<std::vector<int>>>>>> &way) {
        ...
        if (move == 0) return;
            ...
            for (auto elem: F1moves){
                ...
                if (ret == rec(move1, node2, moveSize * flag, pos2, newS1, S2, dp, way)){
                    get_orderings_from_dp(move1, node2, moveSize * flag, pos2, newS1, S2, dp, way);
                    break;
                }
            }
        }
            ...
            for (auto elem: F2moves){
                ...
                if (ret == rec(node1, move2, pos1, moveSize * flag, S1, newS2, dp, way)){
                    get_orderings_from_dp(node1, move2, pos1, moveSize * flag, S1, newS2, dp, way);
                    break;
                }
            }
        }
    ...
    int getTotalSize(int branchFactorF1, int maxDepthF1, int branchFactorF2, int maxDepthF2) {
        if (branchFactorF1*maxDepthF1 + branchFactorF2*maxDepthF2 >= 25) return 1e9;
        long long total_size = F1str->getTotalInstrSize() * 1LL* F2str->getTotalInstrSize(); 
        if (total_size > 1e9) return 1e9;
    ...
    std::map<std::string, int> getOptReorderingDP(int memoryLimit = 5e7) {
        ...
        for (int i = 0; i < n; i++){
            ...
            branchFactorF1 = std::max(branchFactorF1, (int)F1str->nodes[i].getBranches().size());
        }
        for (int i = 0; i < m; i++){
            maxDepthF2 = std::max(maxDepthF2, F2str->nodes[i].getDepth());
            branchFactorF2 = std::max(branchFactorF2, (int)F2str->nodes[i].getBranches().size());
        }
        maxDepthF1 += 1; maxDepthF2 += 1;
        ...
        if (cap){
            while (getTotalSize(best_bFactorF1, best_dF1, best_bFactorF2, best_dF2) > memoryLimit){
                if (best_bFactorF1*1LL*best_dF1 >= best_bFactorF2*1LL*best_dF2){
        ...
        // getTotalInstrSize() vs getTotalSize(best_bFactorF1, best_dF1, best_bFactorF2, best_dF2)
        memo["naive_memory"] = F1str->getTotalInstrSize() * F2str->getTotalInstrSize();
        memo["actual_memory"] = getTotalSize(branchFactorF1, maxDepthF1, branchFactorF2, maxDepthF2);
        memo["dp_memory"] = getTotalSize(best_bFactorF1, best_dF1, best_bFactorF2, best_dF2);
        memo["best_bFactorF1"] = best_bFactorF1;
        memo["best_dF1"] = best_dF1;
        memo["best_bFactorF2"] = best_bFactorF2;
        memo["best_dF2"] = best_dF2;

        if (getTotalSize(best_bFactorF1, best_dF1, best_bFactorF2, best_dF2) > memoryLimit){
            // errs() << "\tERR: DP table too large to handle! Abort DP-based reordering.\n";
            return memo;
        }

        double start_time = clock();
        std::vector<std::vector<std::vector<std::vector<std::vector<std::vector<int>>>>>> dp, way; // node1, node2, pos1, pos2, sig1, sig2
                ...
                for (int k = 0; k < dp[node1][node2].size(); k++){
                    ...
                    for (int l = 0; l < dp[node1][node2][k].size(); l++){
                        ...
                        for (int s1 = 0; s1 < dp[node1][node2][k][l].size(); s1++){
                            dp[node1][node2][k][l][s1].resize(1<<(best_bFactorF2*best_dF2), -1);
                        }
                    }
                }
            }
        ...
        //         Signature(best_dF1, best_bFactorF1), Signature(best_dF2, best_bFactorF2), dp, way) << "\n";

        get_orderings_from_dp(F1str->root, F2str->root, 0, 0, 
                Signature(best_dF1, best_bFactorF1), Signature(best_dF2, best_bFactorF2), dp, way);
        double end_time = clock();
        dp.clear(); dp.shrink_to_fit();
        way.clear(); way.shrink_to_fit();

        return memo;
    }   

    void genFlattening(std::vector <int> &BlockOrd, TreeString* Fstr, SmallVector <Value*, 8> &Fseq) {
...
public:
    TreeStringAlignment() : BaseType(ScoringSystem(-1, 2, -1), nullptr), 
                            F1str(nullptr), F2str(nullptr) {}
    TreeStringAlignment(ScoringSystem Scoring, MatchFnTy Match = nullptr) : 
                        BaseType(Scoring, Match), F1str(nullptr), F2str(nullptr) {}
    ~TreeStringAlignment() {
        if (F1str != nullptr) delete F1str;
        if (F2str != nullptr) delete F2str;
    }

    std::map<std::string, int> getOptFlattening(Function *F1, Function *F2, SmallVector <Value*, 8> &F1seq, SmallVector <Value*, 8> &F2seq) {
        initialize(F1, F2);
    ...
    virtual AlignedSequence <Ty, Blank> getAlignment(ContainerType &F1seq, ContainerType &F2seq) {
