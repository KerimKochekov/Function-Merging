...
static cl::opt<unsigned> SAMethod(
    "func-merging-sa-method", cl::init(0), cl::Hidden,
...
static cl::opt<bool>
    EnableLCS("func-merging-lcs", cl::init(false), cl::Hidden, 
                  cl::desc("Enable optimized branch reordering with LCS"));
    ...
    while (!WorkList.empty()) {
        ...
        while (!Rank.empty()) {
            ...
            if (Result.getMergedFunction() != nullptr && verifyFunction(*Result.getMergedFunction())) {
                ...
                if (EnableLCS and DpResult.getMergedFunction() != nullptr && verifyFunction(*DpResult.getMergedFunction()))
            ...
            if (Result.getMergedFunction() and validFunction) {
                ...
                unsigned DpSizeF12 = EnableLCS ? EstimateThunkOverhead(DpResult, AlwaysPreserved) +
                    EstimateFunctionSize(DpResult.getMergedFunction(), &TTI) : SizeF12;
                ...
                if (Profitable) {
                    ...
                    if (Result.getMergedFunction() != nullptr)
                        Result.getMergedFunction()->eraseFromParent();
                    if (EnableLCS and DpResult.getMergedFunction() != nullptr)
                        DpResult.getMergedFunction()->eraseFromParent();
                } 
