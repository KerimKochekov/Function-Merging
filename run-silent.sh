...
compile_and_measure() {
  ...
  $CLANGPP -std=gnu++98 -x ir ${file} -Os -fno-vectorize -fno-slp-vectorize -fno-unroll-loops -o ${outFile} -c -lstdc++ -lm 2>/dev/null
  if [[ ${mode} == "linked" ]]; then
    local linkedFile="${outDir}/${filename}.out"
    $CLANGPP -z muldefs ${outFile} -lm -o ${linkedFile} 2>/dev/null
    outFile2=${linkedFile}
  fi
  strip -d ${outFile}
...
for arg in "${@:4}"; do
  ...
  $OPT -mergereturn ${inputFile} -o ${outputPath}/_main_._all_._files_._linked_.${format} 2>/dev/null
  ...
  $OPT -mergefunc -func-merging-branch-reord -func-merging-branch-reord-salssa=true -func-merging-branch-reord-whole-program=true -func-merging-branch-reord-explore=${explore} -func-merging-branch-reord-similarity-pruning=false ${inputFileMergedRets} -o ${ourOutLLorBC} 2>/dev/null
  ...
  # SalSSA
  $OPT -mergefunc -func-merging -func-merging-salssa=true -func-merging-whole-program=true -func-merging-explore=${explore} -func-merging-similarity-pruning=false ${inputFileMergedRets} -o ${SalSSAOutLLorBC} 2>/dev/null
  { set +x; } 2>/dev/null
  ...
  echo ${arg} >> ${outputPath}/res.txt
  echo "${baseSz}, ${timeLlvm}" >> ${outputPath}/res.txt
  echo "${ourSz}, ${timeOur}" >> ${outputPath}/res.txt
  echo "${SalSSASz}, ${timeSalSSA}" >> ${outputPath}/res.txt
